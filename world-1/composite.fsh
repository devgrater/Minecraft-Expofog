...
#define OVERWORLD_GROUND_HEIGHT 31
...
void main(){
    ...
    float fogLerpAmount = clamp(pow(eyeAltitude / 40, 2), 0, 1);
    fogLerpAmount *= fogLerpAmount;
    fogLerpAmount *= fogLerpAmount;
    vec4 fogColor = mix(vec4(0.9, 0.3, 0.15, 1.0), vec4(0.6, 0.1, 0.1, 1.0), fogLerpAmount);
